...
//   5) the generated component label is propagated so the dominant IPC-like
//      smooth e-/e+ background and the X17-like signal can be plotted separately.
//
...
namespace
{
    ...
    constexpr double kElectronicNoiseSigmaMeV = 0.020;
    ...
    double SmearEnergy(TRandom3& rng, double e)
    {
        ...
        // Small detector effect: stochastic scintillator resolution plus a tiny
        // electronics-noise term. This modifies measured energies only; it is
        // intentionally not treated as a physics background source.
        const double sigma = std::sqrt(std::pow(kEnergyResolutionRel * e, 2) +
                                       std::pow(kElectronicNoiseSigmaMeV, 2));
        return std::max(0.0, rng.Gaus(e, sigma));
    }
    ...
    bool IsIPC(int componentID)
    {
        return componentID == 0;
    }

    bool IsSignal(int componentID)
    {
                 ...
                 const char* outdir = "plots")
{
    ...
    std::cout << "[analyze_x17] Offline detector effects: silicon pitch = "
              ...
              << kFastAngularSigmaDeg << " deg, electronics noise = "
              << 1000.0*kElectronicNoiseSigmaMeV << " keV" << std::endl;
    ...
    Int_t eventID = -1;
    Int_t componentID = 1;  // 0 = IPC-like background, 1 = X17-like signal; old files default to signal
    ...
    SetAnyBranch(events, {"componentID", "sourceID", "eventType"}, &componentID, false);
    ...
    TH1D* hGenIPC = new TH1D("hGenIPC", "", 90, 0.0, 180.0);
    TH1D* hGenSignal = new TH1D("hGenSignal", "", 90, 0.0, 180.0);
    ...
    TH1D* hDetRecoIPC = new TH1D("hDetRecoIPC", "", 90, 0.0, 180.0);
    TH1D* hDetRecoSignal = new TH1D("hDetRecoSignal", "", 90, 0.0, 180.0);
    ...
    TH1* hists[] = {hGen, hGenIPC, hGenSignal, hIdeal, hGeom, hDetReco, hDetRecoIPC, hDetRecoSignal, hDetAcceptedGen,
    ...
    hGenIPC->SetLineColor(kBlue + 2);
    hGenIPC->SetLineWidth(3);
    hGenIPC->SetLineStyle(1);

    hGenSignal->SetLineColor(kRed + 1);
    hGenSignal->SetLineWidth(3);
    hGenSignal->SetLineStyle(2);
    ...
    hDetRecoIPC->SetLineColor(kBlue + 2);
    hDetRecoIPC->SetLineWidth(3);

    hDetRecoSignal->SetLineColor(kRed + 1);
    hDetRecoSignal->SetLineWidth(3);
    hDetRecoSignal->SetLineStyle(2);
    ...
    reco->Branch("componentID", &componentID, "componentID/I");
    ...
    Long64_t nGeneratedIPC = 0;
    Long64_t nGeneratedSignal = 0;
    Long64_t nIdeal = 0;
    Long64_t nEmSi = 0;
    Long64_t nEpSi = 0;
    Long64_t nGeometry = 0;
    Long64_t nDetector = 0;
    Long64_t nDetectorIPC = 0;
    Long64_t nDetectorSignal = 0;
    ...
    for (Long64_t i = 0; i < nEntries; ++i)
    {
        ...
        if (IsIPC(componentID))
        {
            nGeneratedIPC++;
            hGenIPC->Fill(thetaEE_gen_deg);
        }
        else if (IsSignal(componentID))
        {
            nGeneratedSignal++;
            hGenSignal->Fill(thetaEE_gen_deg);
        }
        ...
        if (passGeometry && energyOK && InRange(thetaEE_detector_deg, 0.0, 180.0))
        {
            ...
            {
                nDetectorIPC++;
                hDetRecoIPC->Fill(thetaEE_detector_deg);
            }
            else if (IsSignal(componentID))
            {
                nDetectorSignal++;
                hDetRecoSignal->Fill(thetaEE_detector_deg);
            }
    ...
    std::cout << "  Generated events        : " << nGenerated
              << "  (IPC-like: " << nGeneratedIPC
              << ", X17-like: " << nGeneratedSignal << ")" << std::endl;
    ...
    std::cout << "  Detector-effect pair    : " << nDetector
              << "  (IPC-like: " << nDetectorIPC
              << ", X17-like: " << nDetectorSignal << ")" << std::endl;
    ...
    // Generator-level physics composition: dominant smooth IPC background plus signal.
    {
        c.Clear();
        const double maxy = 1.35 * std::max({hGen->GetMaximum(),
                                             hGenIPC->GetMaximum(),
                                             hGenSignal->GetMaximum()});
        hGen->GetXaxis()->SetTitle("#theta_{ee}^{gen} [deg]");
        hGen->GetYaxis()->SetTitle("Generated events / 2 deg");
        hGen->SetMinimum(0.0);
        hGen->SetMaximum(maxy > 0.0 ? maxy : 1.0);
        hGen->Draw("HIST");
        hGenIPC->Draw("HIST SAME");
        hGenSignal->Draw("HIST SAME");

        TLegend leg(0.52, 0.68, 0.88, 0.88);
        leg.SetBorderSize(0);
        leg.SetFillStyle(0);
        leg.AddEntry(hGen, "Total generated", "lf");
        leg.AddEntry(hGenIPC, "IPC-like smooth background", "l");
        leg.AddEntry(hGenSignal, "X17-like signal", "l");
        leg.Draw();

        Label("Generated physics composition",
              Form("IPC=%lld, X17=%lld", nGeneratedIPC, nGeneratedSignal));
        c.SaveAs(Form("%s/thetaee_components_generated.pdf", outdir));
    }

    // Detector-level physics composition after geometry and detector effects.
    {
        c.Clear();
        const double maxy = 1.35 * std::max({hDetReco->GetMaximum(),
                                             hDetRecoIPC->GetMaximum(),
                                             hDetRecoSignal->GetMaximum()});
        hDetReco->GetXaxis()->SetTitle("#theta_{ee}^{detector} [deg]");
        hDetReco->GetYaxis()->SetTitle("Detector-effect events / 2 deg");
        hDetReco->SetMinimum(0.0);
        hDetReco->SetMaximum(maxy > 0.0 ? maxy : 1.0);
        hDetReco->Draw("HIST");
        hDetRecoIPC->Draw("HIST SAME");
        hDetRecoSignal->Draw("HIST SAME");

        TLegend leg(0.52, 0.68, 0.88, 0.88);
        leg.SetBorderSize(0);
        leg.SetFillStyle(0);
        leg.AddEntry(hDetReco, "Total detector", "l");
        leg.AddEntry(hDetRecoIPC, "IPC-like smooth background", "l");
        leg.AddEntry(hDetRecoSignal, "X17-like signal", "l");
        leg.Draw();

        Label("Detector-level physics composition",
              Form("IPC=%lld, X17=%lld", nDetectorIPC, nDetectorSignal));
        c.SaveAs(Form("%s/thetaee_components_detector.pdf", outdir));
    }
