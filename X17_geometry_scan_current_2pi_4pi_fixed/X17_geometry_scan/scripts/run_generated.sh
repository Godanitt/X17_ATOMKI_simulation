...
GEOMETRY="${4:-current}"
LOG_DIR="${5:-${PROJECT_DIR}/logs/${GEOMETRY}}"
...
if [[ ! -f "${INPUT}" ]]; then
  ...
  echo "Pass it as the 3rd argument, e.g.: bash scripts/run_generated.sh 10000 sampled.root data/data_pair_creation.txt current" >&2
  exit 2
fi
...
"${BUILD_DIR}/x17sim" -m "${MACRO}" -i "${INPUT}" -o "${OUTPUT}" --mode signal --geometry "${GEOMETRY}" | tee "${LOG_DIR}/run_generated.log"
