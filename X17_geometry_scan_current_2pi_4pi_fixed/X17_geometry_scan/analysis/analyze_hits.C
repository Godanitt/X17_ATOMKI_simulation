...
void analyze_hits(const char* inputFile = "sampled.root",
                  ...
                  const char* geometryName = "unknown")
{
    ...
    TH1D hDetEe("hDetEe", "Electron detector ID;detID;Hits", 128, -0.5, 127.5);
    TH1D hDetEp("hDetEp", "Positron detector ID;detID;Hits", 128, -0.5, 127.5);
    ...
    std::string geometry = geometryName ? geometryName : "unknown";
    Long64_t nGenerated = nGen;
    Long64_t nHitRowsSummary = nHitRows;
    Long64_t nDetectedElectron = static_cast<Long64_t>(eventsWithEe.size());
    Long64_t nDetectedPositron = static_cast<Long64_t>(eventsWithEp.size());
    Long64_t nDetectedCoincidence = nCoincidences;
    double effElectron = effEe;
    double effPositron = effEp;
    double effCoincidence = effCoin;

    TTree summary("summary", "geometrical detection efficiency summary");
    summary.Branch("geometry", &geometry);
    summary.Branch("selectedVolumeID", &selectedVolumeID);
    summary.Branch("nGenerated", &nGenerated);
    summary.Branch("nHitRows", &nHitRowsSummary);
    summary.Branch("nDetectedElectron", &nDetectedElectron);
    summary.Branch("nDetectedPositron", &nDetectedPositron);
    summary.Branch("nDetectedCoincidence", &nDetectedCoincidence);
    summary.Branch("effElectron", &effElectron);
    summary.Branch("effPositron", &effPositron);
    summary.Branch("effCoincidence", &effCoincidence);
    summary.Fill();
    summary.Write();
    ...
    std::cout << "Geometry                     : " << (geometryName ? geometryName : "unknown") << "\n";
